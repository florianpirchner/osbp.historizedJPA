...
public class EmployeeManyToOne {
	...
	@ManyToOne
	@JoinColumns(value={@JoinColumn(name="ADDR_ID", referencedColumnName="ID"), @JoinColumn(name="ADDR_VALIDFROM", referencedColumnName="VALIDFROM")})
	private Address addressRef;

	// @Embedded
	// @AttributeOverrides({ @AttributeOverride(name = "startDate", column =
	...
	public EmployeeManyToOne(int id, String firstName, String lastName) {
		setId(id);
